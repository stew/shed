...
struct TabSlot {
    ...
    activity_seq: u64,
    last_viewed_seq: u64,
    stashed: Option<StashedTab>,
}

impl TabSlot {
    fn new_active(title: Option<String>) -> Self {
        Self {
            title,
            activity_seq: 0,
            last_viewed_seq: 0,
...
impl App {
    ...
    /// Snapshot the App's per-tab fields into a [`StashedTab`]. Used by
    /// `switch_to_tab` to stash the previously-active tab.
    fn stash_active(&mut self) -> StashedTab {
        StashedTab {
    ...
    fn restore_stashed(&mut self, t: StashedTab) {
    ...
    /// handled by [`stash_active`] / [`restore_stashed`].
    fn close_transient_state(&mut self) {
    ...
    fn rename_active_tab(&mut self, new_name: String) {
...
async fn drive_all_tabs(app: &mut App) {
...
/// side-effect of the switch — see `close_transient_state`).
fn try_handle_tab_key(app: &mut App, key: KeyEvent) -> bool {
...
fn begin_rename_tab(app: &mut App) {
    let initial = app
        .tabs
        .get(app.active_tab)
...
fn handle_rename_tab_input_key(app: &mut App, key: KeyEvent) {
    match key.code {
        KeyCode::Enter => {
...
mod tests {
    ...
    fn app_starts_with_one_tab() {
        ...
        // Active tab's stash slot must be None — invariant.
    ...
    fn new_tab_pushes_and_switches() {
        let mut app = App::new();
        app.history.clear();
        // Add a shed in tab 0 so we can verify isolation.
        let _ = app.session.add_shed(vec!["tab0".into()]);
        let idx = app.new_tab();
        assert_eq!(idx, 1);
        assert_eq!(app.tabs.len(), 2);
        assert_eq!(app.active_tab, 1);
        // New tab has a fresh empty session.
        assert!(app.session.sheds().next().is_none());
        // Old tab's session was stashed.
    ...
    fn switch_to_tab_swaps_state_round_trip() {
        ...
        app.history.clear();
        let _ = app.session.add_shed(vec!["a".into()]);
        let _ = app.new_tab(); // now in tab 1
        let _ = app.session.add_shed(vec!["b".into()]);
    ...
    fn cycle_tab_wraps_in_both_directions() {
        let mut app = App::new();
        app.history.clear();
        let _ = app.new_tab(); // tab 1
        let _ = app.new_tab(); // tab 2, active
        assert_eq!(app.active_tab, 2);
        app.cycle_tab(1); // wrap to 0
        assert_eq!(app.active_tab, 0);
        app.cycle_tab(-1); // wrap to 2
        assert_eq!(app.active_tab, 2);
        app.cycle_tab(-1);
        assert_eq!(app.active_tab, 1);
    }
    ...
    fn switch_to_tab_clears_unread_for_destination() {
        let mut app = App::new();
        app.history.clear();
        let _ = app.new_tab(); // active = 1
        // Pretend tab 0 had background activity.
    ...
    fn rename_active_tab_sets_and_clears_title() {
        ...
        // Empty (or whitespace-only) input clears the user-set title.
    ...
    fn display_title_falls_back_to_default_when_unset() {
        let mut app = App::new();
        app.history.clear();
        // No title set, no notebook → "tab 1".
